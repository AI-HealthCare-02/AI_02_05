  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      <div className="bg-gradient-to-br from-violet-600 via-purple-600 to-violet-700 px-5 pt-12 pb-6 text-white">
        <button onClick={() => router.back()} className="text-violet-200 hover:text-white transition-colors text-sm mb-3">‹ 뒤로</button>
        <h1 className="text-xl font-bold">약관 및 정책</h1>
      </div>

      <div className="flex bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 sticky top-0 z-10 transition-colors">
        {[
      ...
      <div className="max-w-md mx-auto px-4 py-4 space-y-3 pb-10">
        <p className="text-xs text-gray-400 dark:text-gray-500">시행일: 2025년 7월 1일</p>
        {items.map((item, i) => (
          <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4 transition-colors">
            <p className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-2">{item.title}</p>
