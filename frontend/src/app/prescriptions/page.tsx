      ...
      <div className="max-w-md mx-auto px-4 py-4 space-y-3">
        ...
        {!loading && list.length === 0 && (
          <div className="bg-white rounded-3xl p-8 text-center shadow-sm">
            ...
            <p className="text-gray-700 font-semibold mb-1">등록된 처방전이 없어요</p>
            <p className="text-gray-400 text-sm mb-4">처방전을 촬영해서 복약 스케줄을 만들어보세요</p>
            <button onClick={() => router.push("/upload")}
              className="bg-violet-600 text-white text-sm px-5 py-2.5 rounded-xl font-medium">
              처방전 등록하기
            </button>
          </div>
        )}
        ...
        {list.map((ocr) => {
          ...
          return (
            <div key={ocr.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
              {/* 처방전 헤더 */}
              <button className="w-full flex items-center gap-3 px-4 py-4"
                ...
                <div className="flex-1 text-left">
                  <p className="text-sm font-bold text-gray-800">{formatDate(date)} 처방</p>
                  <p className="text-xs text-gray-400 mt-0.5">약물 {drugs.length}종</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium
                    ${ocr.status === "done" ? "bg-violet-100 text-violet-600" : "bg-gray-100 text-gray-500"}`}>
              ...
              {isOpen && (
                <div className="border-t border-gray-50">
                    ...
                    {drugs.map((drug, i) => (
                      <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-2.5">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-400">{drug.dosage} · {drug.frequency}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="px-4 pb-4 flex gap-2">
                    <button onClick={() => router.push("/upload")}
                      className="flex-1 text-xs bg-violet-50 text-violet-600 font-semibold py-2.5 rounded-xl hover:bg-violet-100 transition-colors">
                      새 처방전 등록
                    </button>
                    <button onClick={() => handleDelete(ocr.id)}
                      className="flex-1 text-xs bg-red-50 text-red-400 font-semibold py-2.5 rounded-xl hover:bg-red-100 transition-colors">
                      삭제
                    </button>
                  </div>
                </div>
        ...
        {list.length > 0 && (
          <button onClick={() => router.push("/upload")}
            className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-violet-200 rounded-2xl py-4 text-sm text-violet-500 font-medium hover:border-violet-400 hover:bg-violet-50 transition-all">
            + 새 처방전 등록
          </button>
        )}
      </div>
