  ...
  return (
    <main className={`min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200 ${isSenior ? "senior-mode" : ""}`}>
      ...
      <div className="px-4 py-4 space-y-3 max-w-md mx-auto">
        ...
        {/* ✅ 팀원의 재처방 알림 배너 (다크/색약/시니어 모드 완벽 적용) */}
        {isToday && refillAlerts.length > 0 && (
          <div className="space-y-2">
            {refillAlerts.map(({ prescribed_date, items }) => {
              ...
              // 상태에 따른 동적 클래스 적용
              const alertBg = isColorBlind ? "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800" : "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800";
              const alertTitle = isColorBlind ? "text-blue-800 dark:text-blue-300" : "text-amber-800 dark:text-amber-300";
              const alertDesc = isColorBlind ? "text-blue-600 dark:text-blue-400" : "text-amber-600 dark:text-amber-400";

              return (
                <div key={prescribed_date} className={`${alertBg} border rounded-2xl px-4 py-3 flex items-center gap-3 transition-colors`}>
                  <span className={`${isSenior ? "text-2xl" : "text-xl"} flex-shrink-0`}>💊</span>
                  <div className="flex-1">
                    <p className={`${isSenior ? "text-base font-extrabold" : "text-sm font-bold"} ${alertTitle}`}>
                      {diseaseName ? `${diseaseName} 약` : `${formatDate(prescribed_date)} 처방약`}이 곳 떨어져요
                    </p>
                    <p className={`${isSenior ? "text-sm font-bold" : "text-xs"} ${alertDesc}`}>
